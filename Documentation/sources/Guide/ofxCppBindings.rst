...
Four programs in the OpenFX tree use the bindings and serve as their tests:
...
``TestHost/`` in the OpenFX tree is a complete example; its ``README.md`` and
``DESIGN.md`` explain its design.
...
rasterise the result; ``testhost::EffectInstance``, ``TestClip`` and
``TestImage`` (``Effect.h``) hold the pixel buffers; ``Overlay`` and
``CountingInteractInstance`` (``Interact.h``) script an overlay session; and
``Host.cpp`` sets the host's identity and registers its suites.
...
The CMake build compiles the unit tests in ``openfx-cpp/tests``, the test
host, and, with the example plugins on, the plugins that use the bindings. It
