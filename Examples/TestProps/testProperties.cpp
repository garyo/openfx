...
int testPropertySetCompliance(PropertyAccessor& accessor, const char* setName,
  ...
  if (argType == nullptr) {
    ...
    for (const char* propName : setIt->second) {
      ...
      const PropDef* def = find_prop_def(propName);
      if (!def) {
  ...
  for (size_t i = 0; i < propNames.size(); ++i) {
    ...
    try {
      ...
      if (!exists)
        throw openfx::PropertyNotFoundException(kOfxStatErrUnknown, propName);
...
static OfxStatus actionDescribe(OfxImageEffectHandle effect) {
  ...
  plugin::propsets::EffectDescriptor effectDesc(accessor);
...
static void checkMainHandles(const char* action, const void* handle,
  ...
  check("inArgs", inArgsHandle, inArgsCanBeNull);
  check("outArgs", outArgsHandle, outArgsCanBeNull);
}
