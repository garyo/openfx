...
const ParamKindInfo kParamKinds[] = {
    ...
    {kOfxParamTypeDouble, Param::Kind::Double, 1, "ParamDouble1D"},
...
bool EffectInstance::isIdentity(double time, const OfxRectI& window, std::string* identityClip) {
  ...
  // The outArgs set is empty in the metadata until PR #263 lands; define them here.
  PropertySet out = PropertySet::forAction(kOfxImageEffectActionIsIdentity, "outArgs");
  out.define(kOfxPropName, PropertySet::Type::String, 1);
  out.define(kOfxPropTime, PropertySet::Type::Double, 1);
...
std::shared_ptr<ImageBuffer> EffectInstance::render(double time) {
  ...
  // Missing from the metadata until PR #263 lands.
  in.define(kOfxImageEffectPropRenderWindow, PropertySet::Type::Int, 4);
  in.define(kOfxImageEffectPropRenderScale, PropertySet::Type::Double, 2);
  in.define(kOfxImageEffectPropFieldToRender, PropertySet::Type::String, 1);
  {
    auto acc = access(in);
    openfx::propsets::ImageEffectActionRender_InArgs args(acc);
    args.setTime(time).setSequentialRenderStatus(0).setInteractiveRenderStatus(0).setRenderQualityDraft(0).setOpenGLEnabled(0);
    acc.setAll<PropId::OfxImageEffectPropRenderWindow>(std::array{window.x1, window.y1, window.x2, window.y2});
    acc.setAll<PropId::OfxImageEffectPropRenderScale>(std::array{1.0, 1.0});
    acc.set<PropId::OfxImageEffectPropFieldToRender>(kOfxImageFieldNone);
  }
