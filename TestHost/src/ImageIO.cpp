...
std::shared_ptr<ImageBuffer> readImage(const fs::path& path) {
  ...
  if (width <= 0 || height <= 0)
    throw std::runtime_error(path.string() + ": bad image size");
  ...
  if (magic == "P6") {
    ...
    for (int y = height - 1; y >= 0; --y) {  // PPM is top-down; OFX is bottom-up
      ...
      for (int x = 0; x < width; ++x)
        image->setPixel(x, y,
                        {row[x * 3] / float(maxval), row[x * 3 + 1] / float(maxval),
                         row[x * 3 + 2] / float(maxval), 1.f});
    }
    ...
    for (int y = 0; y < height; ++y) {  // PFM is bottom-up, like OFX
      ...
      for (int x = 0; x < width; ++x) {
        ...
        image->setPixel(x, y,
                        channels == 3 ? std::array<float, 4>{p[0], p[1], p[2], 1.f}
                                      : std::array<float, 4>{p[0], p[0], p[0], 1.f});
      }
    }
  } else {
    throw std::runtime_error(path.string() +
                             ": unsupported format (need P6 PPM or PF/Pf PFM)");
  }
  if (!in)
    throw std::runtime_error(path.string() + ": truncated image data");
  return image;
}
  ...
  if (ext == ".pfm") {
    ...
    for (int y = height - 1; y >= 0; --y)
      for (int x = 0; x < width; ++x) {
        auto p = image.pixel(x, y);
        for (int c = 0; c < 3; ++c)
          out.put(static_cast<char>(std::lround(std::clamp(p[c], 0.f, 1.f) * 255.f)));
      }
  }
}

std::shared_ptr<ImageBuffer> solidImage(int width, int height,
...
std::shared_ptr<ImageBuffer> rampImage(int width, int height) {
  ...
  for (int y = 0; y < height; ++y)
    for (int x = 0; x < width; ++x)
      image->setPixel(x, y,
                      {width > 1 ? x / float(width - 1) : 0.f,
                       height > 1 ? y / float(height - 1) : 0.f, 0.5f, 1.f});
  return image;
}
