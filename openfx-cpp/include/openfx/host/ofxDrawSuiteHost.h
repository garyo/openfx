...
// A recording implementation of OfxDrawSuiteV1 (OFX 1.5). Every call a plugin
// makes while drawing an overlay is appended to a list of DrawCommands that
// the host can inspect, print or rasterise; nothing is drawn by the suite
// itself, because a host's display is its own business.
//
...
// The host's palette for OfxDrawSuiteV1::getColour. A host with a real theme
// replaces it; these are the neutral values the test host draws with.
inline const OfxRGBAColourF& standardColour(OfxStandardColour which) {
...
// What a plugin drew, in the order it drew it. The handle a plugin holds is a
// pointer to one of these, as every other OFX handle in this framework is.
class DrawContext {
 public:
  DrawContext() = default;
  ...
  // outside one, as the specification requires. Opening clears the record.
  void open() {
    commands_.clear();
    colour_ = {1, 1, 1, 1};
    lineWidth_ = 0;
  ...
  OfxStatus setColour(const OfxRGBAColourF& c) {
    ...
    colour_ = c;
    record(DrawCommand::Kind::Colour);
    return kOfxStatOK;
  }
  ...
  OfxStatus setLineWidth(float width) {
    if (!open_)
      return kOfxStatFailed;
    lineWidth_ = width;
    record(DrawCommand::Kind::LineWidth);
    return kOfxStatOK;
  }
...
inline void rasterise(const DrawContext& context, const ViewportMapping& mapping,
                      const PlotFn& plot) {
...
// The draw suite over DrawContext: every call is recorded, none is drawn.
