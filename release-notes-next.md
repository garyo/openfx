...
- `openfx-cpp`: the support headers the generated accessors depend on (`ofxPropsAccess.h`, `ofxSpan.h`, logging, exceptions, `SuiteContainer`, `Image`/`Clip` wrappers) are now in the tree; the generated property-set accessor classes moved to the `openfx::propsets` namespace; accessor names no longer collide with C++ keywords or each other (`defaultValue()`, `paramPropType()`), and the host/plugin/instance property families get their natural short names (`setIsBackground()`, `setEffectDuration()`).
