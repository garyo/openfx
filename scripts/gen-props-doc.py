    ...
    with open(outfile_path, "w") as outfile:
        ...
        for type_key, type_name in sorted(types.items()):
            type_props = [
                ...
                if props_metadata[p].get("type") == type_key
                or (
                    isinstance(props_metadata[p].get("type"), list)
                    and type_key in props_metadata[p].get("type")
                )
            ]
            ...
            for prop in sorted(type_props):
                ...
                if dim == 0:
                    outfile.write("- **Dimension**: Variable (0 or more)\n")
                else:
                ...
                if prop in prop_to_sets:
                    ...
                    outfile.write(
                        f"- **Used in Property Sets**: {', '.join(set_parts)}\n"
                    )
                ...
                if metadata.get("introduced"):
                    outfile.write(
                        f"- **Introduced in**: version {metadata.get('introduced')}\n"
                    )

                if metadata.get("deprecated"):
                    outfile.write(
                        f"- **Deprecated in**: version {metadata.get('deprecated')}\n"
                    )
...
def generate_action_args_documentation(action_data, props_metadata, props_by_set_doc):
    ...
    action_section.append(
        "For each action, the required input properties (passed from host to plugin) and output properties "
    )
    action_section.append(
        "(set by the plugin for the host to read) are documented.\n\n"
    )
    ...
    for action_name in sorted(action_data.keys()):
        ...
        if action_data[action_name].get("inArgs"):
            has_args = True
            action_section.append("**Input Arguments**\n\n")
            ...
            for entry in action_data[action_name]["inArgs"]:
                ...
                action_section.append(
                    f"- :ref:`{prop} <prop_{prop}>` - Type: {type_str}, Dimension: {dim_str} (:c:macro:`{cname}`)\n"
                )
        ...
        if action_data[action_name].get("outArgs"):
            has_args = True
            action_section.append("**Output Arguments**\n\n")
            ...
            for entry in action_data[action_name]["outArgs"]:
                ...
                # Write property entry with link to full definition and Doxygen reference
                action_section.append(
                    f"- :ref:`{prop} <prop_{prop}>` - Type: {type_str}, Dimension: {dim_str} (doc: :c:macro:`{cname}`)\n"
                )

                action_section.append("\n")
        if not has_args:
            action_section.append("-- no in/out args --\n\n")
    return "".join(action_section)


def generate_property_set_documentation(
    props_by_set, props_metadata, actions_data, outfile_path
):
    ...
    with open(outfile_path, "w") as outfile:
        ...
        action_docs = generate_action_args_documentation(
            actions_data, props_metadata, props_by_set
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate RST documentation from OpenFX property definitions"
    )
    parser.add_argument(
        "--props-doc",
        default=props_doc_path,
        help="Output path for properties documentation",
    )
    parser.add_argument(
        "--propsets-doc",
        default=propsets_doc_path,
    ...
    if args.verbose:
        log.info(f"Reading properties from headers in {include_dir}")
    ...
    if args.verbose:
        log.info(f"Generating property documentation in {args.props_doc}")
    generate_property_documentation(props_metadata, props_by_set, args.props_doc)

    if args.verbose:
        log.info(f"Generating property sets documentation in {args.propsets_doc}")
    generate_property_set_documentation(
        props_by_set, props_metadata, actions_data, args.propsets_doc
    )
